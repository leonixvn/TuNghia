...
class UpdateVNState(models.Model):
    _inherit = "res.country.state"

    @api.model
    def create_vn_states(self):
        """Xóa và tạo lại danh sách tỉnh/thành Việt Nam"""
        vietnam = self.env['res.country'].search([('code', '=', 'VN')], limit=1)
        if vietnam:
            self.search([('country_id', '=', vietnam.id)]).unlink()  # Xóa các tỉnh cũ
